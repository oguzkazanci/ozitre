...
public class PaymentPdfCreatorServiceImpl implements PaymentPdfCreatorService {
    ...
    public void createEvent(List<EventWithPaymentDto> eventWithPaymentList,
        ...
        List<EventWithPaymentDto> packageAll = new ArrayList<>();
        ...
        List<EventWithPaymentDto> advances = new ArrayList<>();
        List<EventWithPaymentDto> installments = new ArrayList<>();
        for (EventWithPaymentDto ewp : packageAll) {
            String t = ewp.getEvent().getTitle();
            if (isAdvanceTitle(t)) {
                advances.add(ewp);
            } else if (t != null && t.contains("Taksit")) {
                installments.add(ewp);
            }
        }

        Function<PaymentDto, BigDecimal> recv = p -> {
            if (p == null) return BigDecimal.ZERO;
            BigDecimal ar = BigDecimal.valueOf(p.getAmountReceived() == null ? 0L : p.getAmountReceived());
            if (ar.compareTo(BigDecimal.ZERO) > 0) return ar;
            BigDecimal amt = BigDecimal.valueOf(p.getPaymentAmount() == null ? 0L : p.getPaymentAmount());
            BigDecimal rem = BigDecimal.valueOf(p.getRemainingAmount() == null ? 0L : p.getRemainingAmount());
            BigDecimal calc = amt.subtract(rem);
            return calc.compareTo(BigDecimal.ZERO) > 0 ? calc : BigDecimal.ZERO;
        };
        Function<PaymentDto, java.math.BigDecimal> rem = p ->
                BigDecimal.valueOf(p == null || p.getRemainingAmount() == null ? 0L : p.getRemainingAmount());

        Predicate<PaymentDto> notCanceled = p -> {
            Integer st = (p == null ? null : p.getPaymentStatus());
            return st == null || st != 2;
        };

        BigDecimal advanceReceived = advances.stream()
                .map(EventWithPaymentDto::getPayment)
                .filter(notCanceled)
                .map(recv)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        BigDecimal installmentsReceived = installments.stream()
                .map(EventWithPaymentDto::getPayment)
                .filter(notCanceled)
                .map(recv)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        BigDecimal installmentRemainingTotal = installments.stream()
                .map(EventWithPaymentDto::getPayment)
                .filter(notCanceled)
                .map(rem)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        long installmentPaidCount = installments.stream()
                .map(EventWithPaymentDto::getPayment)
                .filter(notCanceled)
                .filter(p -> (p.getPaymentAmount() != null && p.getPaymentAmount() > 0)
                        && (p.getRemainingAmount() != null && p.getRemainingAmount() == 0))
                .count();

        long installmentRemainCount = installments.stream()
                .map(EventWithPaymentDto::getPayment)
                .filter(notCanceled)
                .filter(p -> (p.getPaymentAmount() != null && p.getPaymentAmount() > 0)
                        && (p.getRemainingAmount() == null || p.getRemainingAmount() > 0))
        ...
        BigDecimal paidTotalFixed = advanceReceived.add(installmentsReceived);
        BigDecimal remainingTotalFixed = installmentRemainingTotal;
        ...
        summary.addCell(cellKeyVal("Ödenen Toplam", fmt(paidTotalFixed)));

        summary.addCell(cellKeyVal("Kalan Toplam", fmt(remainingTotalFixed)));
        summary.addCell(cellKeyVal("Ödenen Taksit", String.valueOf(installmentPaidCount)));
        summary.addCell(cellKeyVal("Kalan Taksit", String.valueOf(installmentRemainCount)));
        ...
        totals.addCell(new Cell().add("Toplam").setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
        ...
        totals.addCell(new Cell().add("").setBorder(Border.NO_BORDER)).setMarginLeft(10f);
        totals.addCell(new Cell().add("Kalan Toplam").setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
